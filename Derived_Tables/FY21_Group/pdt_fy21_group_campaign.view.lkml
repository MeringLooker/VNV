view: pdt_fy21_group_campaign {
  derived_table: {
    sql:
      select * from ${pdt_fy21_group_linkedin.SQL_TABLE_NAME}
      union
      select * from ${pdt_fy21_group_sem.SQL_TABLE_NAME}
      ;;
  ...
  dimension: primary_key {
    ...
    hidden: yes
    primary_key: yes
    sql: ${campaign}||'_'||${publisher}||'_'||${placement}||'_'||${creative_name}||'_'||${ad_size}||'_'||${date};;
  }
...
#     drill_fields: []
  ...
  dimension: publisher {
    type: string
    drill_fields: [placement, creative_name, ad_size, date]
    sql: ${TABLE}.publisher ;;
  }
...
#     drill_fields: [publisher,layer,week,month,quarter]
    label: "Referral Rate"
    sql: 1.0*${total_referrals}/nullif(${total_sessions}, 0) ;;
    value_format_name: percent_2
  }
